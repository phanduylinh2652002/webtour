                        ...
                        <td>
                          <div class="d-flex px-2 py-1">
                            <div class="d-flex flex-column justify-content-center">
                              <h6 class="mb-0 text-sm" style="padding-left: 35px">{{$item->tourGuide_id}}</h6>
                            </div>
                          </div>
                        </td>
                        <td>
                            <p class="text-xs font-weight-bold mb-0">{{$item->tourGuide_name}}</p>
                        </td>
                        <td>
                            <p class="text-xs font-weight-bold mb-0" style="text-align: center;">{{$item->tourGuide_email}}</p>
                        </td>
                        <td>
                            <p class="text-xs font-weight-bold mb-0" style="text-align: center;">{{$item->tourGuide_phone}}</p>
                        </td>
                        <td>
                            <p class="text-xs font-weight-bold mb-0" style="text-align: center;">{{$item->tourGuide_address}}</p>
                        </td>
                        <td class="align-middle text-center text-sm d-flex justify-content-center">
                          <a href="{{route('tourguide.show', $item->tourGuide_id)}}" class="btn btn-primary" style="margin-right: 5px">Xem</a>
                          <a href="{{route('tourguide.edit', $item->tourGuide_id)}}" class="btn btn-primary">Sửa</a>
                          <form action="{{route('tourguide.destroy', $item->tourGuide_id)}}" method="post" style="margin-left: 5px">
                            @method('DELETE')
                            @csrf
